...
jest.mock('minio', () => {
  return jest.fn().mockImplementation(() => {
    return {fPutObject: () => {
      return 'cc83c3f2fb5db6561ef4945f6eee031c';
    }};
  });
});
// jest.mock('minio');
...
async function uploadFile() {
  try {
    ...
    const etag=await mockMinioClient.fPutObject(process.env.MINO_BUCKET_NAME, 'testfile', file, metaData);
    console.log(etag);
    return etag;
  } catch (error) {
    console.log(error);
